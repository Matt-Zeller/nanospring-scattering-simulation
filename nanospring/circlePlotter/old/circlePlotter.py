...
#then plots it
    ...
    for i in range(arSize):
        F[i,0]=f(x,1.,0.)
        x=x+h
    #make x axis array, B=b+h in order to include all x in [a,b], not just [a,b-h]
    #without B=b+h, x array is missing the last x index in the set
    B=b+h
    X=np.arange(a,B,h)
